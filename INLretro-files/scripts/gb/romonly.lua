...
--]]
  ...
  for i = 0, 8191, 1 do
    gb.wr(0xa000 + i, i & 0xff)
  end

  for i = 0, 8191, 1 do
    ramval = gb.rd(0xa000 + i)
    if ramval ~= (i & 0xff) then
      print("error", string.format("%X", 0xa000 + i), ramval, i & 0xff)
      break
    end
  end
...
--]]
  ...
  if verify then
    ...
    -- compare the flash file vs post dump file
    if (files.compare(verifyfile, flashfile, true, false)) then
