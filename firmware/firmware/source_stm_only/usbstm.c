...
USBDRIVER void init_usb()
{
  ...
  usb_buff[USB_COUNT0_RX] =
    USB_RX_2TO62_MUL2B(EP0_SIZE); //set EP0 to 8 bytes or whatever it's defined as
...
USBDRIVER static uint16_t standard_req_in(usbRequest_t* spacket)
{
  switch(spacket->bRequest) {
    ...
    case STD_REQ_GET_DESCRIPTOR:
      switch((spacket->wValue & DESC_TYPE_MASK) >>
        8) { //must mask out upper byte and shift to get desc type
...
#define LOG_COUNT	3
      ...
      req_dir =
        (setup_packet->bmRequestType & REQ_DIR_MASK); //set to REQ_DIR_IN/OUT for data stage dir
      ...
      if(((newaddr_reqtype & REQ_TYPE_MASK) == REQ_TYPE_VEND) &&
        ((newaddr_reqtype & REQ_DIR_MASK) == REQ_DIR_OUT)) {
        ...
        JumpToApplication((uint8_t*)&usb_buff[EP0_RX_BASE],
          (usb_buff[USB_COUNT0_RX] & RX_COUNT_MSK));
    ...
    if(req_dir == REQ_DIR_IN) {
      ...
      if(newaddr_reqtype & 0xFF00) {
        ...
        USB->DADDR =
          ((newaddr_reqtype >> 8) | USB_DADDR_EF); //update address and keep USB function enabled
