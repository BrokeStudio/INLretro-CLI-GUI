...
{
  buffer* called_buff =
    &buff0;           //used to point to buffer that was called based on opcode init no warn
...
uint8_t* buffer_payload(setup_packet* spacket, buffer* buff, uint8_t hostsetbuff, uint8_t* rlength)
{
  ...
  if((cur_buff->status == USB_LOADING) && ((spacket->opcode == BUFF_OUT_PAYLOAD_2B_INSP) || (spacket->opcode == BUFF_OUT_PAYLOADN_2B_INSP))) {
...
{
  uint8_t rv = 0;
  if(buff0.status != UNALLOC) {
    rv = 1;
  }
  if(buff1.status != UNALLOC) {
    rv = 2;
  }
#if (defined(NUM_BUFFERS_4) || (defined(NUM_BUFFERS_8)))
  if(buff2.status != UNALLOC) {
    rv = 3;
  }
...
#endif
#ifdef NUM_BUFFERS_8
  if(buff4.status != UNALLOC) {
    rv = 5;
  }
  if(buff5.status != UNALLOC) {
    rv = 6;
  }
  if(buff6.status != UNALLOC) {
    rv = 7;
  }
  if(buff7.status != UNALLOC) {
    rv = 8;
  }
#endif
