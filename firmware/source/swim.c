...
uint8_t swim_call(uint8_t opcode, uint8_t miscdata, uint16_t operand, uint8_t* rdata)
{
  ...
  switch(opcode) {
    ...
    case SWIM_SRST:
      ...
      //assumes low speed
      rdata[RD0] = swim_xfr(0x0000, ((SWIM_WR << 16) | 4), swim_base, 1 << swim_pin);
      break;
    case WOTF:
      rdata[RD_LEN] = BYTE_LEN;
      rdata[RD0] = swim_wotf(SWIM_LS, operand, miscdata);
      break;
    case WOTF_HS:
      rdata[RD_LEN] = BYTE_LEN;
      rdata[RD0] = swim_wotf(SWIM_HS, operand, miscdata);
      break;
    case ROTF:
      ...
      *ret_hword = swim_rotf(SWIM_LS, operand);
      break;
    case ROTF_HS:
      rdata[RD_LEN] = HWORD_LEN;
      //this assignment actually undoes the byte swap
      //first index of data includes NAK/ACK just like write routines which only return ACK/NAK
      //second index of data includes actual byte read back
      *ret_hword = swim_rotf(SWIM_HS, operand);
      break;
    default:
...
uint32_t
swim_xfr(uint16_t data_pb, uint32_t spddir_len, GPIO_TypeDef* swim_base, uint16_t swim_mask)
{
  return 0;
}
