...
```sh
sudo apt-get update
sudo apt-get install make gcc-avr avr-libc wget tar bzip2
```
...
CI uploads the contents of `release/` as the `INLretro-firmware-AV04` artifact. See the [historical release notes](firmware%20release%20notes.txt) for earlier releases.
