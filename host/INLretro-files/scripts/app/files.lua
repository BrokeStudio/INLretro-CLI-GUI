...
local function rd_bin_byte(file)
  ...
  while true do --exit when end of file 1 reached
    ...
    elseif byte_str1 and not byte_str2 then
      if size_must_equal then
        ...
        log.info("Files were not the same size")
...
local function readtill_line(file, token)
...
local function jedec_3ln_2hexstr(file)
