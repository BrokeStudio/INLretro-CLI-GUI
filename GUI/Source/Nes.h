...
/*
    void rom_write_file_callback(std::string path, std::string filename)
    {
      printf("NES::callbackTest\n");

      // parse NES rom file header
      parse_header(rom_write_INLOptions.rom_write_file);

      // set default values
      rom_write_INLOptions.prg_rom_size_kb = header.prgRomSize / 1024;
      rom_write_INLOptions.chr_rom_size_kb = header.chrRomSize / 1024;
      rom_write_INLOptions.mapper_name = mappers[get_mapper_index_by_mapper_id(header.mapperId)].script_name;
    }
*/
  private:
